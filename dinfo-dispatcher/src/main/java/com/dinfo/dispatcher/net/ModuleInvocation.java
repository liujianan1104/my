...
import static com.dinfo.dispatcher.util.ObjectUtil.*;
...
public class ModuleInvocation implements Invocation, Serializable {
    ...
    public Response invoke() {
        if (isEmpty(serviceId))
            return SimpleResponse.buildFailure("service id is empty");
        ...
        try {
            ClassLoader loader = DispatcherContext.getOrAddClassLoader(serviceId);
            if (isNull(loader))
                return SimpleResponse.buildFailure("service id %s is not found", serviceId);
    ...
    public ModuleInvocation setServiceId(String serviceId) {
